<a href="/imoveis/{{ $product->id }}/{{ $product->business->name }} de {{ $product->category->name }} - {{ $product->city->name }}"
    ...
    <img src="{{ asset('uploads/' . $product['default_image']) }}" class="w-100" style="object-fit: cover; height: 14rem;"
        alt="Imagem da Casa">
    <p class="position-absolute px-1 mx-2 rounded-1 end-0 top-0 small bg-white opacity-50"
        style="
    ...
    <div class="card-body pt-2 pb-3 px-3" style="display: flex; flex-direction: column; justify-content: space-between;">
        ...
        <div class="pt-2 d-flex justify-content-between text-xs align-items-center mt-2">
            <p class="card-text text-black fw-semibold text-opacity-75 m-0">R$
                {{ number_format($product['value'], 2, ',', '.') }}</p>
