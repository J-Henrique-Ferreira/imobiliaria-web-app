<a href="/imoveis/{{$product->id}}/{{$product->business->name}} de {{$product->category->name}} - {{$product->city->name}}" class="nav-link shadow-sm border border-1 border-secondary border-opacity-25 overflow-hidden rounded-3 mb-3 card-width zoom_card position-relative bg-white">
    ...
    <div class="card-body pt-2 pb-3 px-3" style="display: flex; flex-direction: column; justify-content: space-between;">
        ...
        <div class="pt-2 d-flex justify-content-between text-xs align-items-center mt-2">
            <p class="card-text text-black fw-semibold text-opacity-75 m-0">R$ {{ number_format($product["value"], 2, ',', '.')}}</p>
